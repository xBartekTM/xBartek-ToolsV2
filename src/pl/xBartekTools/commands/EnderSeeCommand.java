...
public class EnderSeeCommand implements CommandExecutor
{
    ...
    public EnderSeeCommand(final ToolsPlugin plugin) {
        this.plugin = plugin;
        this.plugin.getCommand("endersee").setExecutor((CommandExecutor)this);
    }
    
    public boolean onCommand(final CommandSender sender, final Command cmd, final String label, final String[] args) {
        if (sender instanceof Player) {
            final Player player = (Player)sender;
            if (player.hasPermission("tools.endersee")) {
                if (args.length == 0) {
                    player.sendMessage(this.plugin.color("&cPoprawne uzycie: &7/endersee <gracz>"));
                    return true;
                }
                if (args.length > 0) {
